...
# because number should be least and multiple of 8 & 12, so our number will be multiple of lcm of 8 & 12. ; lcm(8,12)*x
# and because remainder should be 5 so equation will be lcm(8,12)*x +5
# and it should result lowest 4 digit possible so final equation will be lcm*x+5>=1000
...
# count 0 in factorial -> we need to find paris for 0 and 5 in final factorial number.
# to optimize it further we can only find paris of 5's as its occurrence will be less than 2.
# to optimize it further, to find occurrence of 5 as factor in factorial of a number we can use below formula:
# res = floor(n/5)+ floor(n/25)+ ...
# because floor(n/5) will return how many factors of 5 present in number n.
