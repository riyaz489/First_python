...
# ( by combining top chars from 0 to j where j is current col value)
# to target substring(by combining left side chars from 0 to i current row value).

# in below operation we choose minimum value and do +1 with minimum one in current position
# and if value is same so just copy-paste number of minimum steps->
# replace | delete
...
def minDistance(word1, word2):
    ...
    # we knew first row and columns are +1 of prev operation so directly assigning it and avoid adding '' as first element on (0,0) position like we did in example
    ...
    for row in range(1, len(word2)):
        for col in range(1, len(word1)):
            if (word1[col - 1] == word2[row - 1]):
                # we remove same chars and chek operation of prev top left element.
